...
fieldType :: (Eq (Name p)) => Name p -> Fold (StructDef p) (Type p)
fieldType n = structFields . ifolded . ifiltered (const $ (==n) . fst) . _2

unqualifiedFieldType :: (Name p ~ QualifiedName) => UnqualifiedName -> Fold (StructDef p) (Type p)
unqualifiedFieldType n = structFields . ifolded . ifiltered (const $ (==n) . unqualifyName . fst) . _2
