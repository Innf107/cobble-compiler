...
import Language.Cobble.Types (QualifiedName, renamed)
...
compile (Let x e rest) = Assign (renderQName x) (compileExpr e) : compile rest
compile (LetRec f k x body rest) = DefFunction (renderQName f) [renderQName k, renderQName x] (compile body) : compile rest
...
compileVal (C.Var qname) = L.Var (renderQName qname)
compileVal (C.Lambda x y c) = L.Function [renderQName x, renderQName y] (compile c)
compileVal (C.Admin x c) = L.Function [renderQName x] (compile c)
...
renderQName = renameMinus . renamed
    where renameMinus = T.map \case
            '-' -> '_'
            x -> x
