...
import Cobble
import Cobble.Build
import Cobble.Config qualified as Config

import Cobble.Syntax

-- Ugh
import Cobble.Core.Lint (CoreLintError (..))
import Cobble.Typechecker (TypeContext (..), TypeError (..))
...
runCompile CompileCmdOpts{compFile, interfaceFiles, output, target, traceTypes, ddumpRenamed, ddumpPostProcessed, ddumpTC, ddumpCore, skipCoreLint, lintAsError, keepCoreResiduals, compileConfig} = do
    Config.modifyConfig (const compileConfig)
    let opts =
            CompileOpts
                { target
                , interfaceFiles
...
runBuild BuildCmdOpts{projectFile, makeCommand, makeOptions, buildConfig} = do
    Config.modifyConfig (const buildConfig)
    let opts =
            BuildOpts
                { projectFile
                }
...
failWithTypeError (DifferentTCon con1 con2 li cxt) =
    typeError
        [ "Cannot match type constructor '" <> originalName con1 <> "' with '" <> originalName con2 <> "'"
        ]
        li
        cxt
failWithTypeError (CannotUnify t1 t2 lexInfo context) =
    typeError
        ...
        lexInfo
        context
failWithTypeError (Occurs tvar ty li cxt) =
    typeError
        ...
        li
        cxt
failWithTypeError (SkolBinding t1 t2 li context) =
    typeError
        ...
        li
        context
failWithTypeError (Impredicative tv ty li cxt) =
    typeError
        ...
        li
        cxt
failWithTypeError (NoInstanceFor constraint li cxt) =
    typeError
        [ "No instance for '" <> show constraint <> "'"
        ]
        li
        cxt
failWithTypeError (InvalidRowHeadConstr ty li cxt) =
    typeError
...
failWithTypeError (RemainingRowFields fields ty1 ty2 li cxt) =
    typeError
        ...
        li
        cxt
failWithTypeError (MissingRowField field ty1 ty2 li cxt) =
    typeError
...
typeError errorLines li context = do
    ...
    let msg =
            "\ESC[1m\STX"
                <> show li
                <> ": \ESC[38;2;255;0;0m\STXerror:\ESC[0m\ESC[1m\STX\n"
...
showContext (WhenUnifying t1 t2) =
    "When unifying\n    Expected type:    "
        <> ppType t1
        <> "\n    with actual type: "
        <> ppType t2
...
globalConfig :: Parser Config.Config
globalConfig =
    Config.Config
